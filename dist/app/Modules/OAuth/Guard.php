...
class Guard
{
    ...
    public function serve()
    {
        ...
        $response = $this->buildResponse($result['to'], $result['from'], $result['message']);

        if (strpos($response, "QUERY_AUTH_CODE:") !== false){
            return $response;
        }
    ...
    protected function buildResponse($to, $from, $message)
    {
        ...
        if ($message['MsgType'] == 'text') {
            if (strpos($message['Content'], "QUERY_AUTH_CODE:") !== false){
                ...
                $result = sprintf($xmlTpl, $to, $from, time(), 'text', "TESTCOMPONENT_MSG_TYPE_TEXT_callback");
            }
        }else{
            $result = sprintf($xmlTpl, $to, $from, time(), 'text', $message['Event']."from_callback");
        }
        if ($this->isSafeMode()) {
